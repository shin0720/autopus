...
Use the following Autopus skills when they fit the task: `review`, `verification`.
...
- Reference: `.agents/skills/agent-teams/SKILL.md` for full Guardian role definition and team interaction protocol
...
Lead consolidates:
  ...
  4. Send unified issue list to Builder via task result handoff
...
In Agent Teams mode, a Builder agent may request focused review during Phase 2 via `task result handoff`.

```python
# Builder → Guardian (partial review request)
task result handoff(to="guardian", message={
...
task result handoff(to="builder", message={
