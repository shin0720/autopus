...
### Step 1.5: Discovery Inputs (from upstream Step 1.4)

PRD 작성 전 컨텍스트는 `/auto plan` 파이프라인 Step 1.4 (Discovery Interview)에서
이미 수집되어 `.autopus/discovery/{slug}.json`에 저장되어 있습니다.

The PRD generator MUST read this file and use the answers as authoritative input for:

| Discovery Answer | PRD Section |
|---|---|
| `problem_users` | Target Users (Section 3) |
| `tier_structure` | Tier Structure / Pricing (Section 5 or new) |
| `core_mechanism` | Core Mechanism / Algorithm (Section 1 or new) |
| `numeric_ac` | Goals & Success Metrics (Section 2) — ALL numeric values must appear verbatim |
| `output_structure` | Response Schema (new section) — table must be included |
| `tone_prohibited.tone` | Design Tone (new section) — 3 keywords |
| `tone_prohibited.prohibited` | Out of Scope / Prohibited (Section 8) — 3 items |
| `data_strategy` | Data Sources (new section) — mark `[HUMAN TASK]` if manual/mixed |
| `integration_ids` | External Dependencies (new section) — include SoT |
| `definition_of_done` | Completion Criteria (new section) — 3+ verifiable items |

**Rule**: The PRD MUST NOT introduce alternative values for these fields. Any deviation
will be caught by the PRD Quality Gate (Step 1.6) and trigger re-generation.

If the discovery file is absent (e.g., `--skip-prd` was used or pipeline was started
without Step 1.4), the PRD generator MAY proceed with the legacy 6-question Q&A as
fallback, BUT each gap-filled section MUST be marked `[AI-GUESSED — NEEDS CONFIRMATION]`.

See `content/rules/automation-quality.md` R3 for the 10 mandatory PRD items.
