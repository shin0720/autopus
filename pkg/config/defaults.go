		...
		Orchestra: OrchestraConf{
			...
			Providers: map[string]ProviderEntry{
				"claude": {
					...
					PaneArgs:      []string{"--print"},
					PromptViaArgs: true,
					Subprocess:    SubprocessProvConf{SchemaFlag: "--json-schema", OutputFormat: "json"},
				},
				"gemini": {
					Binary:        "gemini",
					Args:          []string{"-m", "gemini-3.1-pro-preview", "-p", "", "--output-format", "json"},
					PaneArgs:      []string{"-m", "gemini-3.1-pro-preview"},
					PromptViaArgs: true,
					Subprocess:    SubprocessProvConf{OutputFormat: "json"},
				},
				"codex": {
					Binary:        "codex",
					Args:          []string{"exec", "--full-auto", "-m", "gpt-5.4", "--output-last-message", "-"},
					PaneArgs:      []string{"-m", "gpt-5.4"},
					PromptViaArgs: true,
					Subprocess:    SubprocessProvConf{SchemaFlag: "--output-schema", OutputFormat: "json"},
				},
			},
