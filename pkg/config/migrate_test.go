...
// MigrateOrchestraConfig does not alter codex PromptViaArgs.
	...
	cfg := &HarnessConfig{
		...
		Orchestra: OrchestraConf{
			...
			Providers: map[string]ProviderEntry{
				...
				"gemini": {Binary: "gemini", Args: []string{"-m", "gemini-3.1-pro-preview", "-p", ""}, PromptViaArgs: true},
			},
			Commands: map[string]CommandEntry{
	...
	cfg := &HarnessConfig{
		...
		Orchestra: OrchestraConf{
			...
			Providers: map[string]ProviderEntry{
				...
				"claude": {Binary: "claude", Args: []string{"--print"}},
				"codex":  {Binary: "codex", Args: []string{"exec", "--full-auto", "-m", "gpt-5.4", "--output-last-message", "-"}, PromptViaArgs: true},
			},
			Commands: map[string]CommandEntry{
