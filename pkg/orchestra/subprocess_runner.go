...
func buildSubprocessArgs(req ProviderRequest) []string {
	...
	// Append schema path only when the provider declares an explicit flag.
	if req.SchemaPath != "" && req.Config.SchemaFlag != "" {
		args = append(args, req.Config.SchemaFlag, req.SchemaPath)
	}

	// Pass prompt via args when configured. If the provider args contain an empty
	// string placeholder, replace the first one in-place so flag/value ordering is
	// preserved (e.g. gemini -p "" --output-format json).
